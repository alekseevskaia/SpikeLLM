...
@torch.no_grad()
def evaluate(lm, args, logger, initial_seed):
    
    results = {}
    results["seed"] = initial_seed
    if args.multigpu:
...
def main():
    ...
    results = evaluate(lm, args,logger, initial_seed)
    print(results)
    # import json

    # def write_json(data, name):
    #     write_root_dir = "./results"

    #     f_write = open(os.path.join(write_root_dir, name), 'w', encoding="utf8")
    #     f_write.write(json.dumps(data, indent=4,ensure_ascii=False))
    #     f_write.close()

    # del results["config"]
    
    # write_json(results, f"{initial_seed}.json")
